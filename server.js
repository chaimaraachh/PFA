...
worker.get("/service/stress", (req, res) => {
  const loadSize = parseInt(req.query.size, 10) || 10000;
  const loadArray = [];
  for (let i = 0; i < loadSize; i++) {
    loadArray.push(new Array(1000).fill("x"));
  }
  const result = loadArray.reduce((acc, val) => acc.concat(val), []).length;
  res.status(200).send({ message: `Stressed with ${result} items` });
});
